...
from codriver.processes import (
...
# A child that announces itself, then idles until interrupted. `trap` proves
# the stop path really delivers SIGINT rather than killing the process
# outright.
SAYS_READY = [
    "bash",
    "-c",
...
NEVER_EXITS = [
    "bash",
    "-c",
    'trap "exit 0" INT; while true; do sleep 0.1; done',
]
...
def _wait_for(
    ...
    raise AssertionError(
        f"{name} stayed {current.state} ({current.detail}), wanted {state}"
    )
...
def test_process_is_initializing_until_it_reports_ready(
    tmp_path: Path,
) -> None:
    spec = ProcessSpec(
        "pose",
...
def test_a_process_without_a_marker_goes_green_after_its_grace(
    tmp_path: Path,
) -> None:
...
def test_stopping_sends_sigint_so_the_child_can_close_cleanly(
    tmp_path: Path,
) -> None:
    spec = ProcessSpec(
        "recording", "Recording", SAYS_READY, ready_pattern="ready"
    )
    manager = _manager([spec], tmp_path)
...
def test_recording_and_rviz_cannot_run_together(tmp_path: Path) -> None:
    specs = [
        ProcessSpec(
            "recording", "Recording", NEVER_EXITS, exclusive_group="bag_io"
        ),
    ...
    with pytest.raises(ProcessConflictError, match="cannot do both"):
        manager.start("rviz")
    # The UI needs to know which one is holding the group, to grey out the
    # other.
...
def test_inference_may_run_alongside_a_recording(tmp_path: Path) -> None:
    specs = [
        ...
        ProcessSpec(
            "detection", "Detection", NEVER_EXITS, ready_after_seconds=0.2
        ),
    ]
    manager = _manager(specs, tmp_path)
    manager.start("recording")
...
def test_log_is_written_to_disk_as_well_as_kept_in_memory(
    tmp_path: Path,
) -> None:
    manager = _manager([ProcessSpec("detection", "Detection", DIES)], tmp_path)
    manager.start("detection")
...
def test_run_command_reports_output_and_exit_code() -> None:
    result = run_command(
        CommandSpec("om", "OM", ["bash", "-c", "echo done; exit 0"])
    )
